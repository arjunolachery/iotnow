...
//variables specific to third program
unsigned long currentTimestamp = millis(); // to implement non-blocking delay
unsigned long previousTimestamp = 0;
const long intervalTimestamp = 30000; //30 second delay
...
void loopThird(){
    currentTimestamp = millis(); // to implement non-blocking delay
    if (currentTimestamp - previousTimestamp >= intervalTimestamp) {
    // save the last time you blinked the LED
    previousTimestamp = currentTimestamp;
    if(WiFi.status()== WL_CONNECTED){
    HTTPClient http;
    http.begin("http://www.iotnow.co.in/send.php"); // url to post data to php script
    http.addHeader("Content-Type", "application/x-www-form-urlencoded"); // type of data to be sent
    String dataToSend = "sensorName=" + name
                          + "&sensorLocation=" + location + "&wifiSsid=" + wifiSsid
                          + "&wifiPassword=" + wifiPassword + "&autoUpdate=" + autoUpdate 
                          + "&programChoice=" + programChoice + "&versionChoice=" + versionChoice
                          + "&MACAddress=" + WiFi.macAddress();
    Serial.println(dataToSend);
 
    // Send HTTP POST request
    int ResultCode = http.POST(dataToSend);
       
    if (ResultCode>0) {
      Serial.println("HTTP Response code: "+ResultCode);
    }
    else {
      Serial.println("Error code: "+ResultCode);
    }
    http.end();
  }
  else {
